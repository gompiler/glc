...
data Cmd = Scan
         ...
         | Codegen
-- | CmdI: Cmd + Inp
data CmdI = CI Cmd Inp
             
parseFile :: Parser Inp
parseFile = FileInp <$> strOption
            ( long "file-path"
              <> short 'f'
              <> metavar "FILEPATH"
...
parseParser = info (CI Parse <$> parseSource)
             (fullDesc
...
prettyParser = info (CI Pretty <$> parseSource)
               (fullDesc
...
symbolParser = info (CI Symbol <$> parseSource)
               (fullDesc
               <> progDesc "Outputs the symbol table to stdout or an error if the table is incomplete."
               <> header "symbol - output the symbol table of a source file")

typecheckParser :: ParserInfo CmdI
typecheckParser = info (CI Typecheck <$> parseSource)
                  (fullDesc
...
codegenParser = info (CI Codegen <$> parseFile)
                (fullDesc
