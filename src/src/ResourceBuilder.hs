...
instance Converter T.FuncDecl FuncDecl where
  convert rc (T.FuncDecl (T.ScopedIdent _ i) sig body) =
    wrap $ injectRet <$-> i <*> convert rc sig <*> wrap (convert rc body)
    where
      wrap = RC.wrap rc
      -- Inject return as last statement if function is void
      -- ensure no falloff in generated bytecode
      injectRet :: Ident -> Signature -> Stmt -> FuncDecl
      injectRet ident sig'@(Signature _ rt) body'@(BlockStmt sl) =
        FuncDecl ident sig' $ case rt of
                                Just _ -> body'
                                Nothing -> BlockStmt $ sl ++ [Return Nothing]
      injectRet ident sig' sl' = FuncDecl ident sig' sl'
